...
| AVC decode | Vendored [rusty_h264](https://github.com/remade-with-rust/rusty_h264) 0.16.0 | BSD-2-Clause, `forbid(unsafe_code)`, built `no_std` with no accel kernels, allocator, knobs or threads. Patched for OpenH264-compatible monochrome, cropping, CAVLC escape limits and CABAC end-of-data. Independent pinned OpenH264 2.6.0 comparisons cover x264-generated profiles, cropping, QP, scaling matrices, deblocking, slices, presets and monochrome; malformed-stream parity is still incomplete. See AVC_DEPENDENCIES.md. |
...
| Remaining codecs/encoders | Open | JPEG arithmetic decoding/encoding, AVC malformed-stream parity, sequences and encoding, HEVC encoding, JPEG2000 encoding/HTJ2K, full VVC and uncompressed formats require further implementation/audit. |
