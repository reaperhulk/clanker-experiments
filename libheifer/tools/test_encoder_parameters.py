#!/usr/bin/env python3
"""Original-header encoder parameter records, historical prefixes, borrowed arrays, raw range flags, optional and aliased outputs."""
...
def corpus():
    ...
    for kind in [-2147483648,-1,0,1,2,3,4,2147483647]:
        for version in [-1,0,1,2,3,2147483647]:
            for count in [-2147483648,-1,0,1,5,2147483647]:
                for pointers in range(4):add([version,kind,255,-2147483648,2147483647,count,pointers,-1])
    for flag in range(256):
        for kind in [1,2,3]:add([1,kind,flag,998,-999,0,0,0])
    rng=random.Random(2304)
    for _ in range(100):add([rng.choice([0,1,2,3]),1,rng.randrange(256),rng.randrange(2**32),rng.randrange(2**32),rng.choice([-1,0,1,5]),rng.randrange(4),0])
    return cases,b''.join(data for _,data in cases)

def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--reference-build',required=True);p.add_argument('--candidate',default='target/release/libheifer.so');p.add_argument('--output',default='.build/encoder-parameters-report.json');p.add_argument('--work');p.add_argument('--sanitize',action='store_true');p.add_argument('--no-leak-check',action='store_true');a=p.parse_args()
    if a.no_leak_check and not a.sanitize:p.error('--no-leak-check requires --sanitize')
    output=Path(a.output);output.unlink(missing_ok=True);work=Path(a.work or ('.build/encoder-parameters-sanitized' if a.sanitize else '.build/encoder-parameters')).resolve();inc=work/'include/libheif';inc.mkdir(parents=True,exist_ok=True);ref=Path(a.reference_build).resolve();(inc/'heif_version.h').write_bytes((ref/'libheif/heif_version.h').read_bytes());libs={'reference':ref/'libheif/libheif.so','candidate':Path(a.candidate).resolve()};hashes={k:hashlib.sha256(v.read_bytes()).hexdigest() for k,v in libs.items()};client=Path('tests/encoder_parameters.c');client_hash=hashlib.sha256(client.read_bytes()).hexdigest();cases,payload=corpus();lines={};env=dict(os.environ)
