#!/usr/bin/env python3
"""Rate/distortion of the built-in HEVC encoder against libheif's x265 plugin (default preset and tune):
both encode the same RGB photographs through libheif at each quality, libheif with libde265 decodes
both, and luma PSNR against the source gives Bjontegaard rate differences. Not run in CI (needs network
access for the Kodak test images)."""
import argparse
import glob
import hashlib
import json
...
def main():
    ...
    p.add_argument('--work', default='.build/hevc-rd')
    p.add_argument('--output', default='.build/hevc-rd-report.json')
    a = p.parse_args()
    from PIL import Image
    work = Path(a.work).resolve()
    work.mkdir(parents=True, exist_ok=True)
    ref = Path(a.reference_build).resolve()
    libs = {'x265': ref / 'libheif/libheif.so', 'candidate': Path(a.candidate).resolve()}
    ...
    for raw, (w, h), _ in images:
        for name in libs:
            ...
            for q in QUALITIES:
                ...
                subprocess.run(args + (['x265'] if name == 'x265' else ['libheifer-hevc']), check=True, capture_output=True)
                subprocess.run([str(work / 'x265'), 'dec', str(heic), str(out)], check=True, capture_output=True)
    ...
    for image, r in results.items():
        x = [t for t in r['x265'] if 28 < t[2] < 50]
    ...
    report = dict(scope=__doc__, qualities=QUALITIES, images={raw.stem: sha for raw, _, sha in images},
                  ...
                  mean_psnr_difference=[float(np.mean([r['candidate'][i][2] - r['x265'][i][2] for r in results.values()]))
