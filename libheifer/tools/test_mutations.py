...
MUTATIONS = [
    ...
    ('jpeg2000_header_trailing_ff', 'vendor/hayro-jpeg2000/src/j2c/segment.rs', 'header_reader.skip_bytes(1)?;', 'header_reader.skip_bytes(0)?;', 'htj2k_errors'),
    ...
    ('htj2k_encode_tag_tree_alias', 'src/htj2k_encoder.rs', 'self.levels[lev as usize].get(i).copied().unwrap_or(0xff)', 'if x >= (self.width + (1 << lev) - 1) >> lev { 0xff } else { self.levels[lev as usize].get(i).copied().unwrap_or(0xff) }', 'htj2k_encoding'),
    ('htj2k_encode_lone_sample', 'src/htj2k_encoder.rs', 'Plane::Float(v) => v.iter_mut().for_each(|s| *s *= 2.0),', 'Plane::Float(v) => v.iter_mut().for_each(|s| *s *= 1.0),', 'htj2k_encoding'),
    ('htj2k_encode_kappa', 'src/htj2k_encoder.rs', 'const IRV_K: f32 = 1.230174104914001;', 'const IRV_K: f32 = 1.2301742;', 'htj2k_encoding'),
    ('htj2k_encode_largest_kmax', 'src/htj2k_encoder.rs', '                u32::from(v >> 11).wrapping_sub(1)\n            };\n            num_bits = num_bits.max(t);', '                u32::from(v >> 11).saturating_sub(1)\n            };\n            num_bits = num_bits.max(t);', 'htj2k_encoding'),
    ('htj2k_encode_sign_overflow', 'src/htj2k_encoder.rs', 'let mut val = (t.wrapping_add(t) & mask) >> p;', 'let mut val = t.wrapping_add(t) >> p;', 'htj2k_encoding'),
    ('htj2k_encode_initial_uvlc', 'src/htj2k_encoder.rs', 'let a = uvlc((u_q0 - 2) as usize);', 'let a = uvlc(u_q0 as usize);', 'htj2k_encoding'),
    ('htj2k_encode_step_rounding', 'src/htj2k_encoder.rs', '(delta * (1 << 11) as f32).round() as i32', '(delta * (1 << 11) as f32).floor() as i32', 'htj2k_encoding'),
    ('htj2k_encode_default_lossy', 'crates/capi/src/builtin_htj2k_encoder.rs', '        reversible: false,\n        num_decompositions: 5,', '        reversible: true,\n        num_decompositions: 5,', 'htj2k_encoding'),
    ('htj2k_encode_default_progression', 'crates/capi/src/builtin_htj2k_encoder.rs', 'progression: htj2k_encoder::RPCL,', 'progression: htj2k_encoder::LRCP,', 'htj2k_encoding'),
    ('htj2k_encode_parameter_kind', 'crates/capi/src/builtin_htj2k_encoder.rs', '        kind: 3,\n        value: ParameterValue {\n            string:', '        kind: 2,\n        value: ParameterValue {\n            string:', 'htj2k_encoding'),
    ...
    ('avc_encode_rounded_size', 'crates/capi/src/builtin_avc_encoder.rs', '((s + 1) & !1).max(64)', '((s + 1) & !1).max(32)', 'avc_builtin_encoding'),
    ('avc_encode_default_tu_depth', 'crates/capi/src/builtin_avc_encoder.rs', 'set_integer(p, c"tu-intra-depth".as_ptr(), 2);', 'set_integer(p, c"tu-intra-depth".as_ptr(), 3);', 'avc_builtin_encoding'),
    ('avc_encode_x264_options', 'crates/capi/src/builtin_avc_encoder.rs', '(n, _) if n.starts_with(b"x264:") => {', '(n, _) if n.starts_with(b"x264:-") => {', 'avc_builtin_encoding'),
...
('jpeg2000_tile_order', 'src/jpeg2000.rs', 'if *next != u16::from(record[6]) {', 'if false && *next != u16::from(record[6]) {', 'jpeg2000_errors'),
    ...
    ('plugin_decode_cached_selection', 'src/decoding.rs', 'if cached.is_some() {', 'if cached.is_some() && false {', 'plugin_decoding'),
    ...
    ('brotli_error_string', 'src/compression.rs', '"_ERROR_FORMAT_PADDING_1",', '"_ERROR_FORMAT_PADDING_2",', 'metadata_compression'),
    ('brotli_unci_method', 'src/uncompressed_compression.rs', 'b"brot" => 5,', 'b"brot" => 4,', 'uncompressed_units'),
    ('brotli_decoder_padding_code', 'vendor/brotli-decompressor/src/decode.rs', 'result = BrotliDecoderErrorCode::BROTLI_DECODER_ERROR_FORMAT_PADDING_1;\n            break;\n          }\n          if s.is_metadata', 'result = BrotliDecoderErrorCode::BROTLI_DECODER_ERROR_FORMAT_PADDING_2;\n            break;\n          }\n          if s.is_metadata', 'metadata_compression'),
    ('brotli_decoder_ring_buffer_growth', 'vendor/brotli-decompressor/src/decode.rs', 'let mut min_size = if s.ringbuffer_size != 0 { s.ringbuffer_size } else { 1024 };', 'let mut min_size = if s.ringbuffer_size != 0 { s.ringbuffer_size } else { 32 };', 'metadata_compression'),
    ('brotli_item_encoding', 'src/items.rs', '5 => c"br".into(),', '5 => c"brotli".into(),', 'items'),
    ('brotli_method_supported', 'crates/capi/src/items.rs', 'matches!(method, 0 | 1 | 3 | 4 | 5)', 'matches!(method, 0 | 1 | 3 | 4)', 'items'),
    ('brotli_unci_writer', 'src/tile_encoding.rs', 'if !matches!(*compression, 3..=5) {', 'if !matches!(*compression, 3 | 4) {', 'tile_encoding'),
    ('brotli_encoder_block_iterations', 'vendor/brotli/src/enc/block_splitter.rs', 'if params.quality < 11 { 3 } else { 10 }', 'if params.quality <= 11 { 3 } else { 10 }', 'items'),
    ('brotli_encoder_population_log2', 'vendor/brotli/src/enc/bit_cost.rs', 'let log2p = log2total - FastLog2(*histo as u64);', 'let log2p = log2total - super::util::FastLog2u16(*histo as u16);', 'items'),
    ('brotli_encoder_stitch', 'vendor/brotli/src/enc/backward_references/hq.rs', 'let i_start = position - Params::max_tree_comp_length() as usize + 1;', 'let i_start = position - Params::max_tree_comp_length() as usize;', 'items'),
    ('brotli_encoder_context_mode', 'vendor/brotli/src/enc/encode.rs', '&self.ringbuffer_.data_mo.slice()[self.ringbuffer_.buffer_index..],', 'self.ringbuffer_.data_mo.slice(),', 'items'),
    ...
    ("avc_mono_chroma", "vendor/rusty_h264-decoder/src/mb16.rs", "                    cleft = [128; 8];", "                    cleft = [127; 8];", "avc"),
    ("avc_mono_chroma_dc", "vendor/rusty_h264-decoder/src/mb16.rs", "chroma8x8_pred(0, true, true, &ctop, &cleft, ccorner)", "chroma8x8_pred(0, cy > 0, cx > 0, &ctop, &cleft, ccorner)", "avc"),
    ...
    ("jpeg_arith_dc_context", "vendor/jpeg-decoder/src/arithmetic.rs", "            12 + sign * 4\n", "            12 + sign * 2\n", "jpeg_pixels"),
    ("jpeg_arith_ac_threshold", "vendor/jpeg-decoder/src/arithmetic.rs", "if k <= usize::from(cond.ac_k[tbl]) { 189 } else { 217 }", "if k < usize::from(cond.ac_k[tbl]) { 189 } else { 217 }", "jpeg_pixels"),
    ("jpeg_arith_refine_sign", "vendor/jpeg-decoder/src/arithmetic.rs", "block[at] = if self.decode(reader, Bin::Fixed)? != 0 { m1 } else { p1 };", "block[at] = if self.decode(reader, Bin::Fixed)? != 0 { p1 } else { m1 };", "jpeg_pixels"),
    ("jpeg_arith_restart_statistics", "vendor/jpeg-decoder/src/decoder.rs", "                            arith.reset(&scan.dc_table_indices, &scan.ac_table_indices, arith_dc, arith_ac);\n                            // A marker left", "                            arith.reset(&scan.dc_table_indices, &scan.ac_table_indices, false, false);\n                            // A marker left", "jpeg_pixels"),
    ("jpeg_arith_resync_other_marker", "vendor/jpeg-decoder/src/decoder.rs", "                                    _ => 3,\n", "                                    _ => 1,\n", "jpeg_errors"),
...
def main():
    ...
    parser.add_argument("--x264-reference-build", default=".build/reference-x264")
