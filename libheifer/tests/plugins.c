...
static heif_error get_string(void* p,const char* n,char* out,int size){printf(" gs%s,%d,%d",n,size,*(int*)p);if(size>0)snprintf(out,(size_t)size,"returned:%s",n);return result();}
