...
allocation accounting and object-specific resource lifetimes. Auxiliary/depth, item-property, six camera-matrix, twenty-two sensor-metadata and thirty-nine component APIs, generic items/compression, twelve TAI timestamp APIs, five metadata writers nine text-item APIs three image-area APIs and thirteen handle color/aspect APIs bring the total to
276 partial functions, with 189 functions missing. Compatibility work comes first;
