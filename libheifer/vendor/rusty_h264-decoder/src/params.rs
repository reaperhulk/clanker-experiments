...
impl Sps {
    ...
    pub fn parse(rbsp: &[u8]) -> Result<Self, DecodeError> {
        ...
        if cropping {
            ...
            // libheifer: openh264's bound (a window may crop to zero samples).
            if (cl as usize).saturating_add(cr as usize) > pic_width_in_mbs * 8
                || (ct as usize).saturating_add(cb as usize) > pic_height_in_mbs * 8
            {
