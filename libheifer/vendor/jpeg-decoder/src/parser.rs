...
pub fn parse_sos<R: Read>(reader: &mut R, frame: &FrameInfo) -> Result<ScanInfo> {
    ...
    for _ in 0 .. component_count {
        ...
        let component_index = match frame.components.iter().position(|c| c.identifier == identifier) {
            Some(value) => value,
            None => return Err(Error::Format(format!("Invalid component ID {} in SOS", identifier))),
        };

        // Each of the scan's components must be unique.
        if component_indices.contains(&component_index) {
            return Err(Error::Format(format!("Invalid component ID {} in SOS", identifier)));
        }
    ...
    let mut spectral_selection_start = read_u8(reader)?;
    // Also utilized as ILV parameter in JPEG-LS.
    let mut spectral_selection_end = read_u8(reader)?;

    let byte = read_u8(reader)?;
    let mut successive_approximation_high = byte >> 4;
    let mut successive_approximation_low = byte & 0x0f;
    ...
    let progression_error = || Error::Format(format!("Invalid progressive/lossless parameters Ss={} Se={} Ah={} Al={}",spectral_selection_start,spectral_selection_end,successive_approximation_high,successive_approximation_low));
    if frame.coding_process == CodingProcess::DctProgressive {
        predictor_selection = Predictor::NoPrediction;
        if spectral_selection_end > 63 || spectral_selection_start > spectral_selection_end || (spectral_selection_start == 0 && spectral_selection_end != 0) || (spectral_selection_start != 0 && component_count != 1) || successive_approximation_low > 13 || (successive_approximation_high != 0 && successive_approximation_high != successive_approximation_low + 1) {
            return Err(progression_error());
        }
    }
    else if frame.coding_process == CodingProcess::Lossless {
        if point_transform >= frame.precision { return Err(progression_error()); }
        ...
        predictor_selection = Predictor::NoPrediction;
        // Sequential JPEG ignores unexpected spectral/refinement parameters.
...
pub fn parse_dht<R: Read>(reader: &mut R, _is_baseline: Option<bool>) -> Result<(Vec<Option<HuffmanTable>>, Vec<Option<HuffmanTable>>)> {
    ...
    while length > 16 {
        ...
        if size == 0 {
            ...
            return Err(Error::Format("Bogus Huffman table definition".to_owned()));
        }
        ...
        if index > 3 { return Err(Error::Format(format!("Bogus DHT index {}",index))); }
        match class {
