...
impl HuffmanDecoder {
    ...
    pub fn decode<R: Read>(&mut self, reader: &mut R, table: &HuffmanTable) -> Result<u8> {
        ...
        if size > 0 {
            ...
            // Native JPEG treats an overlong entropy code as a zero symbol.
    ...
    fn consume_bits(&mut self, count: u8) {
        ...
        if count > self.real_bits { self.insufficient_data = true; }
        self.real_bits = self.real_bits.saturating_sub(count);
    ...
    fn read_bits<R: Read>(&mut self, reader: &mut R) -> Result<()> {
        while self.num_bits <= 56 {
            ...
            if byte == 0xFF {
                ...
                if next_byte != 0x00 {
                    ...
                    match next_byte {
                        ...
                        _    => self.marker = Some(Marker::from_u8(next_byte).ok_or_else(|| Error::Format(format!("unsupported marker 0x{next_byte:02x}")))?),
                    }
            ...
            if self.marker.is_none() { self.real_bits += 8; }
