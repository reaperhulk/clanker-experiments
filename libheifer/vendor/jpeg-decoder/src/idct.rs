    ...
    I::IntoIter: ExactSizeIterator<Item = &'a mut [u8]>,
{
    ...
    for i in 0..8 {
            ...
            && coefficients[i + 56] == 0
        {
            ...
            let Kernel {
                ...
                // constants scaled things up by 1<<13; let's bring them back
    ...
    for (chunk, output_chunk) in temp.chunks_exact(8).zip(output) {
        ...
        // constants scaled things up by 1<<13, plus we had 1<<2 from first
        // loop, plus horizontal and vertical each scale by sqrt(8) so together
        // we've got an extra 1<<3, so 1<<18 total we need to remove.
        // so we want to round that, which means adding 0.5 * 1<<18,
        // aka 131072. Also, we'll end up with -128 to 127 that we want
        // to encode as 0..255 by adding 128, so we'll add that before the shift
        const X_SCALE: i32 = 131072 + (128 << 18);
        ...
        if *rest == [Wrapping(0); 7] {
            let dcterm = jpeg_clamp((jpeg_fsh(*s0) + Wrapping(X_SCALE)) >> 18);
            ...
            output_chunk[0] = jpeg_clamp((x0 + t3) >> 18);
            output_chunk[7] = jpeg_clamp((x0 - t3) >> 18);
            output_chunk[1] = jpeg_clamp((x1 + t2) >> 18);
            output_chunk[6] = jpeg_clamp((x1 - t2) >> 18);
            output_chunk[2] = jpeg_clamp((x2 + t1) >> 18);
            output_chunk[5] = jpeg_clamp((x2 - t1) >> 18);
            output_chunk[3] = jpeg_clamp((x3 + t0) >> 18);
            output_chunk[4] = jpeg_clamp((x3 - t0) >> 18);
        }
    }
}
...
fn kernel_x([s0, s2, s4, s6]: [Wrapping<i32>; 4], x_scale: i32) -> [Wrapping<i32>; 4] {
    ...
    let (t2, t3);
    {
        ...
        let p1 = (p2 + p3) * jpeg_f2f(0.5411961);
        t2 = p1 + p3 * jpeg_f2f(-1.847759065);
        t3 = p1 + p2 * jpeg_f2f(0.765366865);
    }
...
fn kernel_t([s1, s3, s5, s7]: [Wrapping<i32>; 4]) -> [Wrapping<i32>; 4] {
    ...
    let p1 = t0 + t3;
    let p2 = t1 + t2;
    let p5 = (p3 + p4) * jpeg_f2f(1.175875602);

    t0 *= jpeg_f2f(0.298631336);
    t1 *= jpeg_f2f(2.053119869);
    t2 *= jpeg_f2f(3.072711026);
    t3 *= jpeg_f2f(1.501321110);

    let p1 = p5 + p1 * jpeg_f2f(-0.899976223);
    let p2 = p5 + p2 * jpeg_f2f(-2.562915447);
    let p3 = p3 * jpeg_f2f(-1.961570560);
    let p4 = p4 * jpeg_f2f(-0.390180644);
...
// Full-resolution integer IDCT precision and rounding used by the libjpeg oracle.
#[inline]
fn jpeg_f2f(x: f64) -> Wrapping<i32> { Wrapping((x * 8192.0).round() as i32) }
#[inline]
fn jpeg_fsh(x: Wrapping<i32>) -> Wrapping<i32> { x << 13 }

fn jpeg_clamp(value: Wrapping<i32>) -> u8 {
    match (value.0 - 128) & 1023 {
        n @ 0..=127 => (n + 128) as u8,
        128..=511 => 255,
        512..=895 => 0,
        n => (n - 896) as u8,
