...
pub struct Decoder<R> {
    ...
    coefficient_bits: [[i8;64]; MAX_COMPONENTS],
    previous_coefficient_bits: [[i8;64]; MAX_COMPONENTS],
    last_good_mcu_row: usize,
    scan_count: usize,
...
impl<R: Read> Decoder<R> {
    ...
    pub fn new(reader: R) -> Decoder<R> {
        Decoder {
            ...
            coefficient_bits: [[-1;64]; MAX_COMPONENTS],
            previous_coefficient_bits: [[-1;64]; MAX_COMPONENTS],
            last_good_mcu_row: 0,
            scan_count: 0,
                ...
                Marker::SOS => {
                    ...
                    if frame.coding_process == CodingProcess::DctProgressive {
                        for &i in &scan.component_indices {
                            for j in scan.spectral_selection.start.min(1)..scan.spectral_selection.end.max(10) {
                                self.previous_coefficient_bits[i][j as usize] = if scans_processed>0 { self.coefficient_bits[i][j as usize] } else { 0 };
                            }
                            for j in scan.spectral_selection.clone() {self.coefficient_bits[i][j as usize]=scan.successive_approximation_low as i8;}
                        }
                    }
                    ...
                    scans_processed += 1;
                    self.scan_count = scans_processed;
                }
    ...
    fn decode_planes(
        ...
        {
            let smoothing = frame.components.iter().enumerate().all(|(i,c)| self.quantization_tables[c.quantization_table_index].as_ref().is_some_and(|q|crate::smoothing::eligible(&self.coefficient_bits[i], q))) && self.coefficient_bits[..frame.components.len()].iter().any(|bits|bits[1..10].iter().any(|&b|b!=0));
            for (i, component) in frame.components.iter().enumerate() {
                // A declared final scan can end early. Native smoothing may
                // still use the preceding scan's precision below its last good row.
                if self.coefficients_finished[i] == !0 && !smoothing {
                    continue;
                }
                ...
                let coefficients = if smoothing {
                    crate::smoothing::smooth(&self.coefficients[i], component, &quantization_table, &self.coefficient_bits[i], if self.scan_count == 1 { &[-1;64] } else { &self.previous_coefficient_bits[i] }, self.last_good_mcu_row, usize::from(frame.mcu_size.height))
                } else { self.coefficients[i].clone() };
                ...
                let mut tasks = (0..frame.mcu_size.height).map(|mcu_y| {
                    ...
                    let row_coefficients =
                        coefficients[offset..offset + coefficients_per_mcu_row].to_vec();
                    (i, row_coefficients)
                });
                        ...
                        for h_pos in 0..mcu_horizontal_samples[i] {
                            ...
                            if scan.successive_approximation_high == 0 {
                                decode_block(
                                    ...
                                    coefficients,
                                    &mut huffman,
                                    self.dc_huffman_tables.get(scan.dc_table_indices[i]).and_then(Option::as_ref),
                                    self.ac_huffman_tables.get(scan.ac_table_indices[i]).and_then(Option::as_ref),
...
fn decode_block<R: Read>(
    ...
    while index < spectral_selection.end {
        if let Some((value, run)) = huffman.decode_fast_ac(reader, ac_table.unwrap())? {
            ...
            coefficients[UNZIGZAG[index.min(63) as usize] as usize] = value << successive_approximation_low;
            index += 1;
        } else {
            ...
            if s == 0 {
                ...
                coefficients[UNZIGZAG[index.min(63) as usize] as usize] =
