...
impl Upsampler {
    pub fn new(components: &[Component], _output_width: u16, _output_height: u16) -> Result<Upsampler> {
...
fn choose_upsampler(sampling_factors: (u8, u8),
                    ...
                    input_width: u16) -> Result<Box<dyn Upsample + Sync>> {
    let h1 = sampling_factors.0 == max_sampling_factors.0;
    let v1 = sampling_factors.1 == max_sampling_factors.1;
    let h2 = sampling_factors.0 * 2 == max_sampling_factors.0;
    let v2 = sampling_factors.1 * 2 == max_sampling_factors.1;

    if h2 && input_width <= 2 && (v1 || v2) {
        return Ok(Box::new(UpsamplerGeneric {
            horizontal_scaling_factor: 2,
            vertical_scaling_factor: if v1 { 1 } else { 2 },
        }));
    }
    if h1 && v1 {
...
impl Upsample for UpsamplerH2V1 {
    fn upsample_row(&self,
        ...
        for i in 1 .. input_width - 1 {
            let sample = 3 * input[i] as u32;
            output[i * 2]     = ((sample + input[i - 1] as u32 + 1) >> 2) as u8;
            output[i * 2 + 1] = ((sample + input[i + 1] as u32 + 2) >> 2) as u8;
        }

        output[(input_width - 1) * 2] = ((input[input_width - 1] as u32 * 3 + input[input_width - 2] as u32 + 1) >> 2) as u8;
...
impl Upsample for UpsamplerH1V2 {
    fn upsample_row(&self,
        ...
        for i in 0..output_width {
            output[i] = ((3 * input_near[i] as u32 + input_far[i] as u32 + 1 + (row % 2) as u32) >> 2) as u8;
        }
    }
}
...
impl Upsample for UpsamplerH2V2 {
    fn upsample_row(&self,
        ...
        for i in 1 .. input_width {
            ...
            output[i * 2 - 1] = ((3 * t0 + t1 + 7) >> 4) as u8;
            output[i * 2]     = ((3 * t1 + t0 + 8) >> 4) as u8;
        }

        output[input_width * 2 - 1] = ((t1 * 4 + 7) >> 4) as u8;
    }
}
