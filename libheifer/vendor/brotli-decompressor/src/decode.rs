...
fn BrotliAllocateRingBuffer<AllocU8: alloc::Allocator<u8>,
  ...
  // libheifer: grow the ring buffer per metablock like the C decoder
  // (BrotliCalculateRingBufferSize and BrotliEnsureRingBuffer in brotli 1.1.0)
  // instead of sizing it once from the first metablock. The size decides when
  // the decoder flushes, and so which of BLOCK_LENGTH_1 and BLOCK_LENGTH_2 an
  // overlong command reports.
  let _ = input;
  let window_size: i32 = 1 << s.window_bits;
  if s.ringbuffer_size == window_size || s.is_metadata != 0 {
    return true;
  }
  let mut new_ringbuffer_size = window_size;
  let mut min_size = if s.ringbuffer_size != 0 { s.ringbuffer_size } else { 1024 };
  let output_size = if s.ringbuffer.slice().len() == 0 { 0 } else { s.pos } + s.meta_block_remaining_len;
  ...
  if s.ringbuffer_size == new_ringbuffer_size {
    return true;
  }
  let mut ringbuffer = s.alloc_u8
    .alloc_cell((new_ringbuffer_size as usize + kRingBufferWriteAheadSlack as usize +
                 kBrotliMaxDictionaryWordLength as usize));
  if (ringbuffer.slice().len() == 0) {
    return false;
  }
  fast_mut!((ringbuffer.slice_mut())[new_ringbuffer_size as usize - 2]) = 0;
  fast_mut!((ringbuffer.slice_mut())[new_ringbuffer_size as usize - 1]) = 0;
  let old = core::mem::replace(&mut s.ringbuffer, ringbuffer);
  if old.slice().len() != 0 {
    let pos = s.pos as usize;
    s.ringbuffer.slice_mut()[..pos].clone_from_slice(&old.slice()[..pos]);
    s.alloc_u8.free_cell(old);
  }
  s.ringbuffer_size = new_ringbuffer_size;
  s.ringbuffer_mask = new_ringbuffer_size - 1;
  true
}
...
mod tests {
  ...
  fn canny_ring_buffer() {
    // As in C brotli 1.1.0, the smallest canny ring buffer holds 1024 bytes.
    assert_eq!(ringbuffer_size(true), 1024);
    assert_eq!(ringbuffer_size(false), 1 << 16);
  }

  #[test]
  fn ring_buffer_grows_with_output() {
    let mut state = BrotliState::new(::StandardAlloc::default(),
                                     ::StandardAlloc::default(),
                                     ::StandardAlloc::default());
    state.window_bits = 16;
    state.meta_block_remaining_len = 1500;
    assert!(BrotliAllocateRingBuffer(&mut state, &[]));
    assert_eq!(state.ringbuffer_size, 2048);
    state.ringbuffer.slice_mut()[..3].clone_from_slice(b"abc");
    state.pos = 3;
    state.meta_block_remaining_len = 3000;
    assert!(BrotliAllocateRingBuffer(&mut state, &[]));
    assert_eq!(state.ringbuffer_size, 4096);
    assert_eq!(&state.ringbuffer.slice()[..3], b"abc");
  }
