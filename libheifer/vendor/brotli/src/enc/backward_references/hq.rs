...
use crate::enc::util::{FastLog2, FastLog2f64};
// libheifer: the C zopfli cost model is single precision.
...
impl<AllocF: Allocator<floatX>> ZopfliCostModel<AllocF> {
    ...
    fn set_from_literal_costs(
        ...
        for i in 0..BROTLI_NUM_COMMAND_SYMBOLS {
            cost_cmd[i] = FastLog2(11 + i as u64) as floatX;
        }
    ...
    Buckets: PartialEq<Buckets>,
{
        ...
        && position >= Params::max_tree_comp_length() as usize)
    {
        ...
        let i_start = position - Params::max_tree_comp_length() as usize + 1;
...
fn SetCost(histogram: &[u32], histogram_size: usize, literal_histogram: bool, cost: &mut [floatX]) {
    ...
    let log2sum = FastLog2(sum) as floatX;
    ...
    let missing_symbol_cost = FastLog2f64(missing_symbol_sum) as floatX + 2.0;
    for i in 0..histogram_size {
        if histogram[i] == 0 {
