...
fn update_cost_and_signal(
    ...
    // libheifer: scalar double-precision loop, as in C FindBlocks.
    for k in 0..num_histograms32 as usize {
        cost[k] -= min_cost;
        if cost[k] >= block_switch_cost {
            cost[k] = block_switch_cost;
            switch_signal[ix + (k >> 3)] |= (1u32 << (k & 7)) as u8;
        }
    }
}
    ...
    u64: core::convert::From<IntegerType>,
{
    ...
    );
    {
        ...
        let mut cost = allocate::<floatX, _>(alloc, num_histograms);
        ...
        let iters: usize = if params.quality < 11 { 3 } else { 10 };
        ...
        <Alloc as Allocator<floatX>>::free_cell(alloc, cost);
