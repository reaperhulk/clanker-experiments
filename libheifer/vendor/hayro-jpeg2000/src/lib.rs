...
impl<'a> Image<'a> {
    ...
    /// Decode transform-domain-restored components without an unsigned level shift.
    /// This preserves fractional precision until the caller rounds each sample.
    pub fn decode_raw_components<'b>(&'a self, context: &'b mut DecoderContext<'a>) -> Result<&'b [ComponentData]> {
        j2c::decode(self.codestream, &self.header, context, false)?;
    ...
    pub fn decode<'b>(
        ...
        j2c::decode(self.codestream, &self.header, decoder_context, true)?;
        ...
        match entry.mapping_type {
            ...
            ComponentMappingType::Palette { column } => {
                ...
                resolved.push(ComponentData {
                    ...
                    decoded_areas: component.decoded_areas.clone(),
                });
            }
        }
    }
